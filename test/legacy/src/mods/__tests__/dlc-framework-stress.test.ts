...
describe('DLC 框架压力测试（大航海/一战/无线电/二战/飞天魔法/帝国/教国/2077 独立占位包）', () => {
  it('① 单 DLC 挂载：默认 39 系统 +1，装配/步进不崩，内核 behavior 不变，命令可用', () => {
    ...
    expect(m.systemDefs.length).toBe(51); // 注册面
    m.mount(dlcAgeOfSail());
    expect(m.systemDefs.length).toBe(52);
    const sim = new Sim({ seed: 52, pawnCount: 2, registry: m });
    expect(sim.systemIds).toHaveLength(53); // 装配面
  ...
  it('② 八独立 DLC 乱序同时挂：47 系统组合步进不崩（requires 全空 = 清单顺序不承担图约束）', () => {
    ...
    expect(sim.systemIds).toHaveLength(60);
  ...
  it('⑥ DLC 系统与默认系统同一卸载语义：disableSystem 单独禁用一个 DLC 系统后仍可跑', () => {
    ...
    expect(sim.systemIds).toHaveLength(54); // 3 挂 1 禁 → 装配只收 2 个 DLC 系统
