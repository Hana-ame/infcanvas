...
export interface EnemyDef {
  ...
  // 捕食者（2026-08-20 用户设计"哈基米独行叼走"）：独行、目标=最近鼠、接触即叼走逃离——
  // 不走群体袭击/拆家/原地磨血路径；mod 可用 registerEnemy 定义自己的捕食者
  predator?: boolean;
  carrySpeedMul?: number;     // 叼走鼠后的逃跑移速倍率（数据驱动）
  // 冲刺技能（2026-08-20：猫的跳跃/冲刺——周期性向目标方向瞬间位移，越过近身反击圈）
...
export const ENEMIES: Record<string, EnemyDef> = {
  ...
  // 2026-08-20 设计（用户）：哈基米 = 独行捕食者——力量速度远高于鼠鼠（基准 hp40/speed4 →
  // 猫 hp110/speed8），接触鼠直接叼走跑路，不纠缠不拆家。击杀掉落=肉（私有进口袋）
  // 2026-08-20 战斗平衡：hp 90→110（更扛揍）、speed 6.5→8（突围快，玩家更难拦）。
