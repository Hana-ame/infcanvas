// DLC 框架压力测试（2026-08-20 用户点饼"以后要加大航海/一战/无线电/二战/飞天魔法/帝国/教国/
...
// 47 系统组合步进不崩（清单顺序不承担图约束）；③ def 冲突被显式捕获（DLC 不能悄悄覆盖核心）；
...
describe('DLC 框架压力测试（大航海/一战/无线电/二战/飞天魔法/帝国/教国/2077 独立占位包）', () => {
  it('① 单 DLC 挂载：默认 39 系统 +1，装配/步进不崩，内核 behavior 不变，命令可用', () => {
    const m = ModRegistry.default();
    expect(m.systemDefs.length).toBe(49); // 注册面
    m.mount(dlcAgeOfSail());
    expect(m.systemDefs.length).toBe(50);
    const sim = new Sim({ seed: 52, pawnCount: 2, registry: m });
    expect(sim.systemIds).toHaveLength(51); // 装配面
  ...
  it('② 八独立 DLC 乱序同时挂：47 系统组合步进不崩（requires 全空 = 清单顺序不承担图约束）', () => {
    const m = ModRegistry.default();
    ...
    expect(sim.systemIds).toHaveLength(58);
  ...
  it('⑤ 卸载 = 现状不变：不挂 DLC 的默认装配与基线一致（零回归对照）', () => {
    ...
    expect(sim.systemIds).toHaveLength(50);
  ...
  it('⑥ DLC 系统与默认系统同一卸载语义：disableSystem 单独禁用一个 DLC 系统后仍可跑', () => {
    ...
    expect(sim.systemIds).toHaveLength(52); // 3 挂 1 禁 → 装配只收 2 个 DLC 系统
  ...
  it('⑦ 2077 DLC 单挂：义体/斯安威斯坦/义体医生诊所注册面齐活，命令可用，before:craft 锚点插进 production 组', () => {
    ...
    expect(sim.systemIds).toHaveLength(51);
...
function dlcEmpire(): ModPack {
  return {
    ...
    apply(m: ModRegistry): void {
      ...
      m.registerStrategyCard({ id: 'dlc:colony', label: '拓殖令', action: 'walkAndWork', workType: 'build', duration: 90, weight: 8, condition: { kind: 'always' }, reason: '占位' });
    },
  };
}
