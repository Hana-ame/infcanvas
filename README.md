...
**测试**：`npm test`（vitest 118）+ e2e：`node scripts/e2e/run-e2e.mjs scripts/e2e/remote-viewer.mjs`（P1 链路）、`scripts/e2e/remote-delta.mjs`（tick delta 增量帧构成）、`scripts/e2e/reconnect.mjs`（断线重连，自起 vite+server）
