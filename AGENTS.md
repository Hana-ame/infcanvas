...
- **决策节流优化**（2026-08-16）：新增 `tuning.pawn.decisionInterval = 2`——小人每 2 秒才真正抽卡决策一次，冷却中保持上次意图；紧急需求/走路/征召/工作不被节流阻塞。降 CPU（decide 频率降一半），行为响应最多滞后 2 秒。`npm test` 当前 = **511 用例 / 56 文件**；系统/包数不变。
