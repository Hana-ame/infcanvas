...
- 测试：`npm test`（vitest，511 用例 / 56 文件全绿，覆盖插件装卸/依赖图/无限地图/契约/DLC/网络/性能回归）；类型：`npx tsc --noEmit`
...
- **寻路/性能/协议/守卫修复批**（2026-08-16，用户「继续想办法优化」「总之继续修」「大地图分 chunk、2s step 难接受」「一个 step 里做了什么」「这是要做成无限世界的」）：`npm test` 当前 = **536 用例 / 57 文件**；系统/包数不变（28 系统 / 27 包）。主要：① 寻路缓存架构优化（延迟清失败路径、容量 32768+FIFO、数字 key、路径简化）② 锚点缓存 + 短距离跳过航点 ③ needs/san 改用 aura/篝火缓存 ④ 无限世界远距离分段直线导航回退 ⑤ 猫新增冲刺技能（EnemyDef.dash + dashCd，周期性越过近身反击圈）⑥ 守卫猫不再无敌（敌方猫可反击）⑦ server 快照透传 taming/owner/dashCd。
